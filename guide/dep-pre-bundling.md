...
::: warning Note
Les dépendances liées peuvent ne pas fonctionner correctement dans la compilation finale à cause de différences dans la façon de résoudre les dépendances.
Utilisez plutôt `npm package` pour toutes les dépendances locales pour éviter d’avoir des problèmes avec la compilation finale.
:::
